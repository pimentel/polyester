...
simulate_experiment = function(fasta=NULL, gtf=NULL, seqpath=NULL,
    ...
    counts_df <- data.frame(matrix(unlist(numreadsList), ncol = length(numreadsList)))
    colnames(counts_df) <- 1:sum(num_reps)
    counts_df$target_id <- .fasta_prefix(names(transcripts))
    size_df <- data.frame(size, target_id = counts_df$target_id)
    colnames(size_df)[1:ncol(size)] <- 1:ncol(size)

    invisible(list(counts = counts_df, size = size_df))
}

.fasta_prefix <- function(seq_names, split_on = " ") {
  unlist(lapply(strsplit(seq_names, split_on), function(x) x[1]))
}
